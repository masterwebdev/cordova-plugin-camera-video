...
//video alex
#define BeginVideoRecording     1117
#define EndVideoRecording       1118
...
- (void) invokeTakePicture:(CGFloat) width withHeight:(CGFloat) height withQuality:(CGFloat) quality{
    ...
    [self.sessionManager.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef sampleBuffer, NSError *error) {
      ...
      //NSLog(@"sfxlog Done creating still image");

      //NSLog(@"sfxlog w %@",width);
      //NSLog(@"sfxlog h %@",height);
      ...
      if (error) {
        ...
        CGFloat trgRatio = width/height;
        CGFloat srcRatio = capturedImage.size.width/capturedImage.size.height;

        CGFloat sx=0;
        CGFloat trgHeight=capturedImage.size.width/trgRatio;

        //if(trgRatio > srcRatio){
          sx=(capturedImage.size.height-trgHeight)*0.5;
        //}

        UIImage *srcImage = nil;
        CGRect cropRect = CGRectMake(0,sx,capturedImage.size.width,trgHeight ); //set your rect size.
        srcImage = [self croppIngimageByImageName:capturedImage toRect:cropRect];
        ...
        if(width > 0 && height > 0){
          ...
          [resizeFilter setValue:[[CIImage alloc] initWithCGImage:[srcImage CGImage]] forKey:kCIInputImageKey];
          ...
          capturedCImage = [[CIImage alloc] initWithCGImage:[srcImage CGImage]];
        }
...
- (void) startRecord:(CDVInvokedUrlCommand*)command {
    ...
    if (self.cameraRenderController != NULL) {
        self.onRecordStartHandlerId = command.callbackId;
        
        CGFloat width = (CGFloat)[command.arguments[0] floatValue];
        CGFloat height = (CGFloat)[command.arguments[1] floatValue];
        CGFloat quality = (CGFloat)[command.arguments[2] floatValue] / 100.0f;
        
        [self invokeStartRecord:width withHeight:height withQuality:quality];
    } else {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Camera not started"];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
    }
...
- (void) stopRecord:(CDVInvokedUrlCommand*)command {
    ...
    if (self.cameraRenderController != NULL) {
        self.onRecordStopHandlerId = command.callbackId;
        
        NSLog(@"STOP CALLBACKID %@", command.callbackId);
        
        [self invokeStopRecord];
    } else {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Camera not started"];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
    }
...
- (void) invokeStartRecord:(CGFloat) width withHeight:(CGFloat) height withQuality:(CGFloat) quality{
    ...
    self.sessionManager.isRecording=YES;
    
    [self.cameraRenderController makeWriter];
    
    //<--

    NSMutableArray *params = [[NSMutableArray alloc] init];
    
    //[params addObject:@"started"];
	
	NSString *vp=self.sessionManager.videoPath;

	if (vp == (id)[NSNull null] || vp == 0 ){
		NSString *guid = [[NSUUID new] UUIDString];
		NSString *uniqueFileName = [NSString stringWithFormat:@"%@/%@.mov", @"Documents", guid];
		vp = [NSHomeDirectory()stringByAppendingPathComponent:uniqueFileName];
	}
	
	if (vp == (id)[NSNull null] || vp.length == 0 ){
		vp=[NSString stringWithFormat:@"%@/%@", NSHomeDirectory(), @"sftemp.mov"];
	}
	
    [params addObject:vp];
    self.sessionManager.videoPath=vp;
    
    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:params];
    [pluginResult setKeepCallbackAsBool:true];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:self.onRecordStartHandlerId];
    
    
}

- (void) invokeStopRecord {
    ...
    [self.cameraRenderController stopWriter];
...
- (void) stopRecordFinished {
    ...
    NSLog(@"Got Stop %@", self.onRecordStopHandlerId);
    
    NSLog(@"VIDEO PATH %@", self.sessionManager.videoPath);
    ...
    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:params];
    //[pluginResult setKeepCallbackAsBool:true];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:self.onRecordStopHandlerId];

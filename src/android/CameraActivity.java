package com.cordovaplugincameravideo;
...
public class CameraActivity extends Fragment {
  ...
  public interface CameraVideoListener {
	...
	//video alex
    void onRecordStarted(String message);
    void onRecordStartedError(String message);
    void onRecordStopped(String message);
    void onRecordStoppedError(String message);
  }

  private CameraVideoListener eventListener;
  ...
  public void setEventListener(CameraVideoListener listener){
  ...
  private void createCameraVideo(){
    if(mPreview == null) {
      setDefaultCameraId();
  ...
  PictureCallback jpegPictureCallback = new PictureCallback(){
    public void onPictureTaken(byte[] data, Camera arg1){
      Log.d(TAG, "CameraVideo jpegPictureCallback");
      ...
      try {
        ...
        Log.d(TAG, "CameraVideo pictureTakenHandler called back");
      } catch (OutOfMemoryError e) {
        // most likely failed to allocate memory for rotateBitmap
        Log.d(TAG, "CameraVideo OutOfMemoryError");
        // failed to allocate memory
        eventListener.onPictureTakenError("Picture too large (memory)");
      } catch (IOException e) {
        Log.d(TAG, "CameraVideo IOException");
        eventListener.onPictureTakenError("IO Error when extracting exif");
      } catch (Exception e) {
        Log.d(TAG, "CameraVideo onPictureTaken general exception");
      } finally {
        canTakePicture = true;
        mCamera.startPreview();
      }
  ...
  private Camera.Size getOptimalPictureSize(final int width, final int height, final Camera.Size previewSize, final List<Camera.Size> supportedSizes){
    ...
    */
    ...
    Log.d(TAG, "CameraVideo previewAspectRatio " + previewAspectRatio);
    ...
    for (int i = 0; i < supportedSizes.size(); i++) {
      ...
      if (supportedSize.equals(requestedSize)) {
        Log.d(TAG, "CameraVideo optimalPictureSize " + supportedSize.width + 'x' + supportedSize.height);
    ...
    Log.d(TAG, "CameraVideo optimalPictureSize " + size.width + 'x' + size.height);
    return size;
  }
  ...
  public void takePicture(final int width, final int height, final int quality){
    Log.d(TAG, "CameraVideo takePicture width: " + width + ", height: " + height + ", quality: " + quality);
  ...
  public void startRecordP(final String filePath, final String camera, final int width, final int height, final int quality, final boolean withFlash){
    Log.d(TAG, "CameraVideo startRecord camera: " + camera + " width: " + width + ", height: " + height + ", quality: " + quality);
    Activity activity = getActivity();
  ...
  //video alex
  public void startRecord(final int width, final int height, final int quality){
  	Log.d(TAG, "CameraVideo startRecord width: " + width + ", height: " + height + ", quality: " + quality);
  		...
  		new Thread() {
  			public void run() {
  				...
  				if (prepareVideoRecorder()) {
  					...
  					try{
  						Log.d(TAG, "CameraVideo START START START");
  						mMediaRecorder.start();
  						isRecording = true;
  					}catch(Exception e){
  						Log.d(TAG, "CameraVideo START ERROR:" + e.getMessage());
  						eventListener.onRecordStartedError(mOutputFile.getAbsolutePath());
  					}
  				} else {
  					releaseMediaRecorder();
  				}

  				eventListener.onRecordStarted(mOutputFile.getAbsolutePath());
  ...
  private File getVideoFile() {
  	/*File folder = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "SmartFlow");
  	folder.mkdirs();
    ...
    String videoFileName=path + "/cpcp_video_" + UUID.randomUUID().toString().replace("-", "").substring(0, 8) + ".mp4";
  ...
  public void stopRecord(){
  	Log.d(TAG, "CameraVideo stopRecord");
  		...
  		public void run() {
  			try {
  				...
  				mMediaRecorder.setPreviewDisplay(null);
	                ...
	                //mOutputFile.delete();
	            	eventListener.onRecordStoppedError("RuntimeException: " + e.getMessage());
	            }
	            ...
	            eventListener.onRecordStopped(mOutputFile.getAbsolutePath());
...
private boolean prepareVideoRecorder(){
	...
	try {
		...
		Log.d(TAG, "OUTPUTFILE MediaRecorder: " + mOutputFile.getAbsolutePath());
      ...
      mMediaRecorder.setVideoEncodingBitRate(1048576);
      mMediaRecorder.setVideoFrameRate(30);*/
      
      mMediaRecorder.setPreviewDisplay(mPreview.mHolder.getSurface());
      
      //mMediaRecorder.setOutputFile(Environment.getExternalStorageDirectory().getPath() +"/test1video.mp4");

      Log.d(TAG, "MediaRecorder: 113");
      
      mMediaRecorder.setOutputFile(mOutputFile.getAbsolutePath());

      Log.d(TAG, "MediaRecorder: 114");
     // mMediaRecorder.setVideoEncodingBitRate(1048576);
      //mMediaRecorder.setVideoFrameRate(30);
      ...
      //mMediaRecorder.setProfile(CamcorderProfile.get(mCamera.,CamcorderProfile.QUALITY_HIGH));
      
      
      //mMediaRecorder.setVideoSize(1280, 768);
      
      String manufacturer = Build.MANUFACTURER.toLowerCase();
      String model = Build.MODEL.toLowerCase();
      
      Log.d(TAG, "CameraVideo MAN " +manufacturer);
      Log.d(TAG, "CameraVideo MOD " +model);
      Log.d(TAG, "CameraVideo MOD " +model.indexOf("t820"));
      
      /*if(manufacturer.indexOf("samsung")>=0){
    	  
      }
      
      if(model.indexOf("t820")>=0){
    	  
      }
      
      if(manufacturer.indexOf("samsung")>=0 && (model.indexOf("t820")>=0 || model.indexOf("t825")>=0)){*/

    	  //rotation new version xx3935
      ...
      //releaseCamera();
      	...
      	Log.d(TAG, "Prepare MediaRecorder: " + e.getMessage());
      }
      ...
      try {
      	...
      	releaseMediaRecorder();
      	eventListener.onRecordStartedError("IllegalStateException preparing MediaRecorder: " + e.getMessage());
      	return false;
      } catch (IOException e) {
      	Log.d(TAG, "IOException preparing MediaRecorder: " + e.getMessage());
      	releaseMediaRecorder();
      	eventListener.onRecordStartedError("IOException preparing MediaRecorder: " + e.getMessage());
      	return false;
      } catch (Exception e) {
      	Log.d(TAG, "Exception preparing MediaRecorder: " + e.getMessage());
      	releaseMediaRecorder();
      	eventListener.onRecordStartedError("Exception preparing MediaRecorder: " + e.getMessage());
      	return false;
      }
      return true;
  }

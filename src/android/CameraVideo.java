...
public class CameraVideo extends CordovaPlugin implements CameraActivity.CameraVideoListener {
  ...
  //video alex
  private static final String START_RECORD_ACTION = "startRecord";
  private static final String STOP_RECORD_ACTION = "stopRecord";
  private static final String REQUEST_PERMISSIONS_ACTION = "requestPermissions";
  private static final String REFRESH_PREVIEW = "refreshPreview";
  private static final int AUD_REQ_CODE = 1;
  ...
  //video alex
  private CallbackContext startRecordCallbackContext;
  private CallbackContext stopRecordCallbackContext;
  private CallbackContext requestPermissionsCallbackContext;
  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
    ...
    } else if (START_RECORD_ACTION.equals(action)) {//video
    return startRecord(args.getInt(0), args.getInt(1), args.getInt(2), callbackContext);
  } else if (STOP_RECORD_ACTION.equals(action)) {
    return stopRecord(callbackContext);
  } else if (REQUEST_PERMISSIONS_ACTION.equals(action)) {
  ...
  public void onSnapshotTakenError(String message) {
    Log.d(TAG, "CameraVideo onSnapshotTakenError");
  ...
  public void onPictureTakenError(String message) {
    Log.d(TAG, "CameraVideo onPictureTakenError");
  ...
  public void onStartRecordVideoError(String message) {
    Log.d(TAG, "CameraVideo onStartRecordVideo");
 ...
 private boolean getHorizontalFOV(CallbackContext callbackContext) {
  ...
  public void onFocusSetError(String message) {
    Log.d(TAG, "CameraVideo onFocusSetError");
...
//video alex
  	  private boolean startRecord(int width, int height, int quality, CallbackContext callbackContext) {
	    if(this.hasView(callbackContext) == false){
	      return true;
	    }
	    
	    startRecordCallbackContext = callbackContext;

	    fragment.startRecord(width, height, quality);

	    return true;
	  }

	  public void onRecordStarted(String message) {
	    ...
	    PluginResult pluginResult = new PluginResult(PluginResult.Status.OK, data);
	    pluginResult.setKeepCallback(true);
	    startRecordCallbackContext.sendPluginResult(pluginResult);
	  }

	  public void onRecordStartedError(String message) {
	    Log.d(TAG, "CameraVideo onRecordStartedError");
	    startRecordCallbackContext.error(message);
	  }
	  
  	  private boolean stopRecord(CallbackContext callbackContext) {
  		  
  		Log.e(TAG,"start record 1");
  		  
	    if(this.hasView(callbackContext) == false){
	      return true;
	    }

	    stopRecordCallbackContext = callbackContext;
	  ...
	  public void onRecordStopped(String message) {
	    ...
	    PluginResult pluginResult = new PluginResult(PluginResult.Status.OK, data);
	    pluginResult.setKeepCallback(true);
	    stopRecordCallbackContext.sendPluginResult(pluginResult);
	  }

	  public void onRecordStoppedError(String message) {
	    Log.d(TAG, "CameraVideo onRecordStoppedError");
	    stopRecordCallbackContext.error(message);
	  }
	    ...
	    //requestPermissionsCallbackContext = callbackContext;

	    try{
	    	
	    	/*if (!cordova.hasPermission(Manifest.permission.RECORD_AUDIO)) {

		    	cordova.requestPermissions(this, 0, [android.Manifest.permission.RECORD_AUDIO]);
	    	...
	    	if (cordova.hasPermission(permissions[1])) {
	            ...
	            cordova.requestPermissions(this, AUD_REQ_CODE, permissions);
	          }

	        //}
	    }catch(Exception e){
	    	//stopRecordCallbackContext.error("error");
	    }
